...
_TITLES = {p["slug"]: p["title"].replace('"', "&quot;") for p in POSTS}
COVERS = {s: f'<img class="cover" src="{u}" alt="{_TITLES.get(s, "")}" loading="lazy" referrerpolicy="no-referrer">' for s, u in _PHOTOS.items()}
...
for _s in ("measuring-geo","llms-txt","ai-hallucination-brand","global-buyer-ai","export-voucher-pr","renewal-case","press-case"):
    COVERS[_s] = f'<img class="cover" src="{_PHOTOS[_s]}" alt="{_TITLES.get(_s, "")}" loading="lazy" referrerpolicy="no-referrer">'  # SVG 대신 사진 커버

# admin CMS: 글 데이터에 photo 필드가 있으면 그 사진을 커버로 사용 (비어 있으면 위 기본값/SVG 유지)
for _p in POSTS:
    if _p.get("photo"):
        COVERS[_p["slug"]] = f'<img class="cover" src="{_p["photo"]}" alt="{_TITLES.get(_p["slug"], "")}" loading="lazy" referrerpolicy="no-referrer">'
...
"indexed-not-cited": """<figure class="fig"><svg viewBox="0 0 600 196" xmlns="http://www.w3.org/2000/svg" font-family="'Pretendard',system-ui,sans-serif"><rect x="22" y="40" width="216" height="112" rx="14" fill="#fff" stroke="#E5EAF2"/><circle cx="58" cy="76" r="15" fill="#0BBF8C"/><path d="M52 76l4 4 8-9" stroke="#fff" stroke-width="2.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/><text x="84" y="72" font-size="15.5" fill="#0A1930" font-weight="800">색인됨</text><text x="84" y="90" font-size="11" fill="#8B95A7" font-weight="700" font-family="Poppins,sans-serif">INDEXED</text><text x="42" y="126" font-size="12.5" fill="#8B95A7">검색엔진이 페이지를 수집·저장</text><path d="M250 96 h98" stroke="#B7C0D0" stroke-width="2.4" stroke-dasharray="4 6" fill="none" stroke-linecap="round"/><path d="M342 90 l9 6 -9 6" stroke="#B7C0D0" stroke-width="2.4" fill="none" stroke-linecap="round" stroke-linejoin="round"/><text x="300" y="82" font-size="11" fill="#E2574C" text-anchor="middle" font-weight="800">신뢰의 관문</text><rect x="362" y="40" width="216" height="112" rx="14" fill="#fff" stroke="#C7D6FF"/><rect x="380" y="61" width="30" height="30" rx="9" fill="#2B5CFF"/><text x="395" y="81" font-size="12" fill="#fff" text-anchor="middle" font-weight="800" font-family="Poppins,sans-serif">AI</text><text x="422" y="72" font-size="15.5" fill="#0A1930" font-weight="800">인용됨</text><text x="422" y="90" font-size="11" fill="#2B5CFF" font-weight="700" font-family="Poppins,sans-serif">CITED</text><text x="382" y="126" font-size="12.5" fill="#8B95A7">AI가 답의 근거로 선택·인용</text></svg><figcaption>색인은 시작일 뿐, 인용까지 가야 AI 답변에 노출됩니다</figcaption></figure>""",
...
<div><h2>우리 회사는 AI에서 어떻게 보이고 있을까요?</h2><p>AI 가시성 무료 진단 · 보도자료 1건 무료 · 최적화 콘텐츠 3건</p></div>
