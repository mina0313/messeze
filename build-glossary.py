...
<a href="{p}services.html"><b style="color:var(--cobalt)">구독형 AI PR 패키지 →</b><span>진단·홈페이지·콘텐츠·언론을 월 단위로 함께 운영</span></a>
...
# ---------------- next-t 인사이트 반영 (기존에 없던 개념만 추가) ----------------
t("topic-cluster","onpage","토픽 클러스터","Topic Cluster",
"하나의 핵심 주제(필러)를 중심에 두고 세부 주제 글들을 내부 링크로 엮어 '이 주제의 권위 있는 곳'임을 보여주는 콘텐츠 구조.",
["넓은 주제를 다루는 대표 글(필러)과 그 주제의 구체적 질문에 답하는 여러 세부 글(클러스터)을 서로 링크로 연결합니다. 검색엔진과 AI는 이 묶음을 보고 '이 사이트가 이 주제를 깊이 있게 다룬다'고 판단합니다.",
"글을 하나씩 흩어 쓰는 것보다 한 주제를 필러+세부 구조로 촘촘히 덮는 편이 훨씬 강한 신호를 만듭니다. 검색량이 적은 B2B 주제일수록 '이 분야는 이 회사'라는 인상을 만드는 데 효과적입니다."],
"messeze는 고객의 핵심 질문을 필러로 잡고 파생 질문들을 세부 콘텐츠로 매달 채워 토픽 클러스터를 완성합니다.",
["internal-link","keyword-research","information-gain"])

t("cannibalization","onpage","키워드 카니발라이제이션","Keyword Cannibalization",
"우리 사이트의 여러 페이지가 같은 키워드를 두고 서로 경쟁해, 검색엔진이 어느 페이지를 올려야 할지 헷갈리는 상태.",
["비슷한 주제의 글을 여러 개 쓰다 보면 같은 키워드를 겹쳐 노리게 됩니다. 그러면 검색엔진은 어느 페이지가 대표인지 판단하지 못해 순위가 분산되고, 결국 둘 다 낮게 노출되는 손해가 생깁니다.",
"해결책은 페이지를 합치거나 캐노니컬로 대표를 지정하고, 각 페이지의 검색 의도를 명확히 구분하는 것입니다. 많이 쓰는 것보다 겹치지 않게 쓰는 것이 중요합니다."],
"messeze는 콘텐츠를 축적할 때 질문(의도)별로 역할을 나눠, 같은 키워드로 서로 잡아먹는 일이 없게 설계합니다.",
["keyword","canonical","keyword-research"])

t("keyword-research","basic","키워드 리서치","Keyword Research",
"고객이 실제로 검색하는(=AI에게 묻는) 단어와 질문을 조사해 어디에 힘을 쏟을지 정하는 출발 작업.",
["검색량·경쟁도·검색 의도를 함께 보면서 우리가 이길 수 있고 전환으로 이어지는 키워드를 골라냅니다. 무작정 큰 키워드를 노리기보다 구체적인 롱테일 질문을 잡는 편이 B2B에서는 훨씬 유리합니다.",
"AI 검색 시대에는 '키워드'보다 '질문 문장' 리서치에 가깝습니다. 고객이 AI에게 던질 법한 질문을 뽑아내는 것이 곧 전략의 뼈대가 됩니다."],
"messeze 온보딩의 첫 단계가 바로 이 리서치 — 고객이 AI에게 물어볼 핵심 질문 5~6개를 함께 설계합니다.",
["keyword","search-intent","topic-cluster"])

t("technical-seo","basic","테크니컬 SEO","Technical SEO",
"콘텐츠 내용이 아니라, 검색엔진·AI가 사이트를 잘 읽고 색인할 수 있게 만드는 기술적 토대 작업.",
["크롤링·색인, 사이트 속도, 구조화 데이터, 모바일 대응, robots·사이트맵 같은 '뒷단' 요소를 정비합니다. 아무리 좋은 콘텐츠도 크롤러가 못 읽거나 느리면 노출되지 않습니다.",
"테크니컬 SEO는 눈에 잘 안 보이지만, 이게 무너지면 콘텐츠·신뢰 신호가 아무리 쌓여도 전달되지 않는 '기반 공사'입니다."],
"messeze는 홈페이지 정비 단계에서 메타·헤딩·스키마·속도 같은 테크니컬 요소를 함께 손봐 AI가 읽는 구조를 만듭니다.",
["crawling-indexing","core-web-vitals","sitemap"])

t("javascript-seo","basic","JavaScript SEO","JavaScript SEO",
"자바스크립트로 그려지는(동적 렌더링) 페이지도 검색엔진·AI 크롤러가 제대로 읽게 만드는 최적화.",
["요즘 사이트는 화면을 자바스크립트로 나중에 그리는 경우가 많습니다. 그런데 일부 크롤러는 자바스크립트 실행을 건너뛰거나 늦게 처리해, 정작 중요한 콘텐츠를 '빈 페이지'로 볼 수 있습니다.",
"핵심 콘텐츠는 서버에서 미리 그려 보내거나(SSR·SSG) 크롤러가 볼 수 있는 형태로 제공하는 것이 안전합니다. 특히 AI 크롤러는 자바스크립트를 실행하지 않는 경우가 많아 더 중요합니다."],
"messeze는 신규 구축·리뉴얼 시 핵심 정보가 크롤러에게 텍스트로 전달되는지 확인해 'AI가 못 읽는 페이지'를 방지합니다.",
["crawling-indexing","rendering","ai-crawler"])

t("rendering","basic","렌더링 방식 (CSR·SSR·SSG)","Rendering (CSR · SSR · SSG)",
"페이지를 어디서 그리느냐 — 브라우저(CSR)·서버(SSR)·미리 생성(SSG) — 에 따라 검색·AI 노출이 달라지는 방식.",
["CSR은 브라우저에서 자바스크립트로 그려 크롤러가 놓치기 쉽고, SSR은 서버가 완성된 HTML을 보내 크롤러가 바로 읽을 수 있으며, SSG는 미리 만들어 둔 정적 페이지라 가장 빠르고 안정적입니다.",
"SEO·AI 노출이 중요한 페이지일수록 SSR이나 SSG로 '완성된 문서'를 내보내는 것이 유리합니다. 렌더링 방식 선택이 곧 노출 여부를 가르기도 합니다."],
"messeze는 노출이 중요한 페이지를 크롤러가 바로 읽을 수 있는 방식으로 구성해 AI 가시성을 확보합니다.",
["javascript-seo","crawling-indexing","page-speed"])

t("site-migration","basic","사이트 이전","Site Migration",
"도메인 변경·리뉴얼·구조 개편처럼 사이트를 옮길 때, 그동안 쌓인 검색 신뢰를 잃지 않게 넘기는 작업.",
["URL이 바뀌면 기존에 쌓인 순위·백링크·색인이 끊길 수 있습니다. 301 리다이렉트로 옛 주소를 새 주소에 정확히 연결하고, 사이트맵·캐노니컬을 갱신해 검색엔진이 이전을 이해하게 해야 합니다.",
"이전은 잘하면 티가 안 나지만, 잘못하면 트래픽이 한순간에 사라지는 위험한 작업입니다. 계획 없이 리뉴얼했다가 노출이 통째로 날아가는 경우가 흔합니다."],
"messeze는 홈페이지 리뉴얼·이전 시 리다이렉트와 색인 이전을 함께 챙겨, 쌓아온 AI·검색 신뢰가 끊기지 않게 합니다.",
["redirect","canonical","sitemap"])

t("zero-click","ai","제로클릭 검색 (ZCS)","Zero-Click Search",
"검색 결과나 AI 답변 안에서 궁금증이 바로 해결돼, 사용자가 어떤 링크도 클릭하지 않고 끝나는 검색.",
["AI 요약·지식패널·바로답변이 늘면서 사람들은 링크를 누르지 않고 화면 위에서 답을 얻고 떠납니다. 클릭(트래픽)은 줄지만, 그 답 안에 '우리 회사가 인용되느냐'가 새로운 승부처가 됩니다.",
"즉 트래픽이 줄어도 문제가 아니라, 답변 자체에 등장하지 못하는 것이 진짜 문제입니다. 노출의 기준이 '클릭 수'에서 'AI 답변 속 인용'으로 옮겨가고 있습니다."],
