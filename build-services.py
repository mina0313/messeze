...
.pttags span{font-size:12px;font-weight:700;color:var(--cobalt);background:var(--sky);border-radius:999px;padding:5px 11px}
...
s(slug="visibility", no="01", title="AI 가시성 평가", en="AI Visibility Assessment",
  ...
  items=[("엔진별 인식 상태 점검","주요 AI 검색 4곳에 동일한 질문 세트를 던져 인식·인용 여부와 답변 내용을 기록합니다. '어느 엔진이 우리를 아는가'가 지도에 그려집니다.",["4개엔진","AI질의테스트","인용확인"]),
        ("핵심 질문 5~6개 설계","고객이 실제로 물어볼 질문을 온보딩 미팅에서 함께 정의합니다. 이 질문들이 이후 홈페이지·콘텐츠·언론 실행 전체의 뼈대가 됩니다.",["질문설계","온보딩미팅","실행뼈대"]),
        ("홈페이지 구조 진단","타이틀·헤딩·메타 같은 SEO 기본기부터 구조화 데이터·엔티티 연결까지, AI 크롤러 관점에서 읽히는 상태를 점검합니다.",["SEO기본기","구조화데이터","크롤러관점"]),
        ("경쟁사 비교 분석","같은 질문에서 경쟁사가 어떤 출처 덕분에 답변에 등장하는지 분해합니다. 따라잡을 목록이 그대로 실행 과제가 됩니다.",["경쟁사분해","출처분석","실행과제"]),
        ("개선 우선순위 로드맵","점수를 가장 많이 올릴 항목부터 순서대로. 직접 실행해도 되고, messeze에 맡겨도 되는 형태로 작성합니다.",["우선순위","로드맵","셀프실행OK"]),
        ("기존 콘텐츠·기사 자산 조사","이미 보유한 기사·블로그·자료 중 AI가 활용할 수 있는 자산과 방치된 자산을 구분합니다.",["자산조사","기사아카이브","재활용"])],
...
s(slug="website-renewal", no="02", title="홈페이지 수정·리뉴얼", en="Website Optimization & Renewal",
  ...
  items=[("메타·헤딩 구조 정비","페이지마다 고유한 타이틀과 디스크립션, H1→H2→H3 위계를 바로잡습니다. 검색과 AI가 문서 구조를 즉시 파악하게 됩니다.",["타이틀정비","헤딩위계","메타"]),
        ("구조화 데이터(JSON-LD) 탑재","Organization·Service·FAQPage 스키마로 발행 주체와 서비스를 기계가 읽는 형식으로 선언합니다. @id·sameAs 연결까지.",["JSON-LD","스키마","sameAs연결"]),
        ("질문-답변형 페이지 신설","설계한 핵심 질문에 정면으로 답하는 페이지를 홈페이지 안에 만듭니다. AI 인용의 착지 지점이 됩니다.",["질문페이지","AI인용착지","FAQ"]),
        ("이미지·텍스트 리커버리","이미지 속에 갇힌 정보를 텍스트로 꺼내고, 모든 이미지에 의미 있는 ALT를 채웁니다.",["ALT완비","텍스트화","정보복구"]),
        ("크롤링 기반 정비","robots.txt·sitemap.xml·canonical을 정리해 검색엔진과 AI 크롤러의 수집 경로를 엽니다.",["robots","sitemap","canonical"]),
        ("전후 점수 검증","수정 전후를 AI 가시성 체크 점수로 비교해 개선을 숫자로 증명합니다.",["전후비교","점수증명","리포트"])],
...
s(slug="website-build", no="03", title="홈페이지 제작", en="AEO-first Website Build",
  ...
  items=[("질문 기반 정보 구조(IA) 설계","핵심 질문 → 페이지 구조로 변환합니다. 산업·서비스·질문별로 페이지를 계속 늘릴 수 있는 확장형 골격입니다.",["정보구조IA","질문기반","확장형골격"]),
        ("AEO·SEO 기본기 내장","메타·헤딩·구조화 데이터·사이트맵·robots까지, 나중에 고칠 것 없이 처음부터 표준으로 들어갑니다.",["표준마크업","스키마내장","기본기"]),
        ("블로그·콘텐츠 시스템 포함","운영 단계에서 칼럼이 쌓일 수 있도록 발행 구조와 템플릿을 함께 짓습니다.",["발행구조","콘텐츠축적","템플릿"]),
        ("반응형·속도 최적화","모바일 우선 인덱싱과 사용자 경험 지표(Core Web Vitals)를 기준으로 만듭니다.",["모바일우선","CoreWebVitals","속도"]),
        ("다국어 확장 대비","수출기업이라면 베트남어·영어·중국어 페이지로 확장 가능한 컴포넌트 구조로 설계합니다.",["다국어","수출대비","확장설계"]),
        ("런칭 후 색인 등록","서치콘솔·네이버 서치어드바이저 등록과 초기 색인 확인까지 마무리합니다.",["서치콘솔","네이버등록","초기색인"])],
...
s(slug="own-blog", no="04", title="홈페이지 내 블로그 운영", en="Owned Blog Operation",
  ...
  items=[("질문 기반 칼럼 기획","'고객이 AI에게 물어볼 질문'이 곧 글감입니다. 검색량이 아니라 질문에서 출발해 글감이 고갈되지 않습니다.",["질문이글감","글감무한","검색량무관"]),
        ("AEO형 구조로 집필","질문형 소제목 → 정면 답변 → 근거 순서. 사람도 AI도 원하는 답을 바로 찾는 구조입니다.",["질문소제목","정면답변","근거순서"]),
        ("전문성 소재 인터뷰","현장의 노하우를 짧은 인터뷰·자료로 받아 전문가의 글로 바꿉니다. 쓸 사람이 없어도 됩니다.",["현장인터뷰","전문가의글","대신집필"]),
        ("FAQ·Article 스키마 적용","발행 글마다 구조화 데이터를 붙여 리치 결과와 AI 인용 자격을 만듭니다.",["FAQ스키마","Article","리치결과"]),
        ("발행 캘린더 운영","월 단위 발행 계획으로 꾸준히. 축적의 리듬이 곧 전략입니다.",["월간발행","축적리듬","꾸준함"]),
        ("성과 추적","글별 노출·유입과 AI 인용 변화를 월간 리포트에 담습니다.",["노출유입","AI인용변화","월간리포트"])],
...
s(slug="channels", no="05", title="외부 채널 운영", en="External Channel Operation",
  ...
  items=[("네이버 블로그 — 국내 검색","국내 B2B 발주 담당자가 여전히 가장 많이 쓰는 네이버 검색. 신뢰 콘텐츠로 네이버 노출을 잡습니다.",["네이버검색","국내B2B","신뢰콘텐츠"]),
        ("티스토리 — 구글·다음","구글 검색에 잘 잡히는 플랫폼. 기술·정보성 콘텐츠의 구글 유입을 만듭니다.",["구글유입","다음검색","정보성글"]),
        ("구글 블로거 — 빠른 색인","구글 소유 플랫폼 특유의 빠른 인덱싱. 신규 소식을 구글 생태계에 빠르게 심습니다.",["빠른색인","구글생태계","신규소식"]),
        ("원본-변주 체계","자사 블로그 원본을 채널별로 다시 씁니다. 복붙 중복이 아니라 서로를 보강하는 교차 출처가 되게.",["변주콘텐츠","중복회피","교차출처"]),
        ("채널 개설·세팅 대행","채널이 없다면 개설부터 프로필·카테고리 세팅까지 대행합니다.",["개설대행","프로필세팅","카테고리"]),
        ("채널별 유입 리포트","채널별 노출·유입을 월간 리포트로 비교해 잘 되는 채널에 힘을 싣습니다.",["채널비교","유입분석","월간리포트"])],
...
s(slug="press", no="06", title="언론 배포", en="Press Distribution",
  ...
  items=[("보도자료 기획·작성","리드문 승부, 검증 가능한 사실 중심. 기사화 가능성과 AI 인용 가능성 두 기준으로 씁니다.",["리드문승부","사실중심","AI인용기준"]),
        ("업종별 기자 매칭","산업 담당 기자 데이터베이스를 기반으로 맞는 출입처에 정확히 배포합니다. 스팸이 아니라 제보가 되게.",["기자DB","출입처매칭","제보형배포"]),
        ("애드버토리얼 발행","게재가 보장되는 기사형 콘텐츠로 원하는 메시지를 원하는 시점에 싣습니다.",["게재보장","기사형콘텐츠","원하는시점"]),
        ("뉴스거리 발굴","신제품·인증·수출·전시회 — 회사 안의 소식에서 기사가 될 이야기를 함께 찾습니다.",["신제품·인증","수출·전시회","소재발굴"]),
        ("발행 결과 관리","배포·게재 결과를 추적하고, 발행된 기사를 홈페이지·채널과 연결해 자산화합니다.",["게재추적","자산화","채널연결"]),
        ("해외 언론 확장","수출기업이라면 베트남·중국·미국 등 현지어 보도자료와 현지 매체 배포까지 확장합니다.",["현지어원고","베트남·중국","해외매체"])],
  proc=[("뉴스거리 발굴","월간 미팅에서 보도 소재를 정리합니다.",["신제품·인증","수출·전시회","소재발굴"]),
