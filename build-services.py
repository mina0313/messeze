...
.cal{position:absolute;right:-2%;bottom:4%;background:#fff;border:1px solid var(--line);border-radius:14px;box-shadow:var(--sh-lg);padding:16px 18px;transform:rotate(2deg);z-index:4}
.cal .cl{font-size:11px;font-weight:800;color:var(--mut);margin-bottom:10px}
.cal .dots{display:grid;grid-template-columns:repeat(7,14px);gap:7px}
.cal .dots i{width:14px;height:14px;border-radius:5px;background:#EEF2F9}
.cal .dots i.on{background:var(--cobalt)}
...
.clips .clip.c1{top:8%;left:0;width:56%;transform:rotate(-3.5deg);z-index:1}
.clips .clip.c2{top:26%;right:2%;width:58%;transform:rotate(1.6deg);z-index:3;border-color:#C4D4FF;box-shadow:0 26px 56px rgba(16,31,63,.18)}
...
/* process */
.proc{background:var(--sky-2)}
.pr-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;counter-reset:pr}
.pr{background:#fff;border:1px solid var(--line);border-radius:18px;padding:24px 22px;position:relative}
.pr .n{font-family:var(--disp);font-weight:700;font-size:14px;color:#fff;background:var(--ink);width:34px;height:34px;border-radius:11px;display:grid;place-items:center;margin-bottom:14px}
...
@media(max-width:980px){
  .nav-menu{display:none}.mega{display:none}
...
<div class="cal"><div class="cl">이번 달 발행</div>
<div class="dots"><i></i><i class="on"></i><i></i><i></i><i class="on"></i><i></i><i></i><i></i><i></i><i class="on"></i><i></i><i></i><i class="on"></i><i></i></div></div>
</div>""",
...
s(slug="press", no="06", title="언론 배포", en="Press Distribution",
  ...
  faq=[("보도자료를 보내면 기사화가 보장되나요?","보도자료는 기자의 판단을 거치므로 보장되지 않습니다. 그래서 뉴스 가치를 살린 원고와 정확한 기자 매칭으로 확률을 높이고, 반드시 실어야 하는 메시지는 게재가 보장되는 애드버토리얼로 조합합니다."),
       ...
       ("해외 배포는 어떻게 진행되나요?","목표 국가의 언어로 현지화한 보도자료를 현지 매체·기자에게 배포합니다. 베트남·중국 등 국가별 매체망을 활용하며, 기업형 플랜 또는 별도 견적으로 진행합니다.")],
